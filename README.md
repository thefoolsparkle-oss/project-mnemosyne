...
- 第三阶段“表达系统”已推进第一版轻表达闭环：回复可携带至多一个白名单结构化轻表达标签，由程序单独展示，并已限制连续出现和近期同标签重复；暂不扩张到完整贴纸或形象资源。
...
- 输出净化与表达层：括号动作不直接进入聊天文本；当前支持经白名单校验的结构化轻表达标签并存入 `message_expressions`，后续再由表情/动作调度决定是否映射贴纸、头像表情或动作。
...
- 轻表达标签：人格回复在确有情绪承接需要时可附带至多一个白名单结构化标签（如轻声、微笑、点头），后端从正文剥离并持久化，普通端在气泡旁以克制的小标签展示；上一条回复已显示表达时本轮抑制展示，近期相同标签也不重复；括号动作继续只作为兼容清理路径
...
- 第三阶段轻表达验证：`py scripts/verify_phase3_expression.py` 使用临时数据库检查结构化标签白名单、无效标签清理、括号兼容提取、程序标签不泄漏到正文、持久化回读、连续展示冷却与近期同标签抑制
